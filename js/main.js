...
// * (Optional) the maximum number of items to be displayed, as an integer
const fillListFromAPI = (listTopic, element, maxItems) =>
{
  const numItems = maxItems <= 10 ? maxItems : 10;
  let updatedNewsList = '';

  fetch(`http://content.guardianapis.com/search?show-fields=all&section=${listTopic}&api-key=9wur7sdh84azzazdt3ye54k4`)
    ...
    for (let i = 0; i < numItems; i++) {
      updatedNewsList +=
        `<li>
            <a href="${results[i].webUrl}">${results[i].webTitle}</a>
            <div>${results[i].fields.trailText}</div>
        </li>`;
    }
  }).catch(function (ex) {
